...
## Part A — Wire the RabbitMQ publisher _(~50 min)_
...
## Part B — Register notification-service in the gateway _(~20 min)_
...
```bash
curl http://localhost:8004/v1/notifications
```
...
- What happens to the activity request if `notification-service` is down when the message is published? Should the activity creation fail?
  **Answer** shouldnt fall beacuse RabbitMQ should catch it and keep it until the notification-service is up and running again. the activity is the main action so it should be saved. the notification is a side effect, so it shouldn't block the user.
- In Module 3, you called `game-service` directly over HTTP to enrich the response. Why not do the same for notifications — why introduce a broker at all?
  **Answer** decoupliing and async communication. notifications shound't block the user's actions.
- The activity is saved and the message is sent — but you have no confirmation the notification was delivered. What visibility do you lose compared to a synchronous call?
  **Answer** immediate confirmation that the notification was consumed, processed and delivered.
